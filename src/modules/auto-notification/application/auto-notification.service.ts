...
export class AutoNotificationService {
  ...
  create(dto: CreateAutoNotificationDto) {
    return this.prisma.autoNotification.create({
      data: dto,
    });
  }
  ...
  findOne(id: string) {
    return this.prisma.autoNotification.findUnique({
      where: { id },
    });
  }

  update(id: string, dto: UpdateAutoNotificationDto) {
    return this.prisma.autoNotification.update({
      where: { id },
      data: dto,
    });
  }
  ...
  remove(id: string) {
    return this.prisma.autoNotification.delete({
      where: { id },
    });
  }
}
